...
public:
    AlgorithmsClient(const std::string& h = "localhost", int p = 8081) 
    ...
    void run() {
        ...
        std::cout << "Connected to Multi-Algorithms server at " << host << ":" << port << std::endl;
        std::cout << "Available algorithms: euler, clique, mst, hamilton" << std::endl;
        std::cout << "Type 'quit' to exit, 'help' for format, 'list' for algorithms" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    AlgorithmsClient client(host, port);
    client.run();
